...
# LABEL CONVENTION — MUST match the HuggingFace dataset
# (garystafford/deepfake-audio-detection): 0 = real/human, 1 = fake/AI.
# The previous version had these INVERTED (AI=0, Human=1), which contradicted
# the 1,866 HF samples and made the model unable to learn AI-vs-human at all.
LOCAL_FILES = [
    ("text_fraud_recognition/hebrew_3_ai.mp3", 1),       # AI  -> 1
    ("text_fraud_recognition/speach_hebrew_ai.mp3", 1),  # AI  -> 1
    # speach_hebrew_1.mp3 intentionally REMOVED: it is a long edited mix
    # (human speech + background, multiple segments). Two independent detectors
    # disagree on it (conformer + wav2vec2 -> 79% AI). Not a clean single-class
    # sample, so it poisons training. Re-add only if cleanly re-labelled.
]
# All "28 мая" files are real human calls
import glob
for f in sorted(glob.glob("text_fraud_recognition/28*.mp3")):
    LOCAL_FILES.append((f, 0))   # human -> 0
# Clean human test calls (skipped automatically if not present / not decodable)
for _extra in ("text_fraud_recognition/test_call_human.m4a",
               "text_fraud_recognition/test_call_human_he.m4a"):
    if os.path.isfile(_extra):
        LOCAL_FILES.append((_extra, 0))

# ── Drop-in data folders ──────────────────────────────────────────────────────
# Anything you put in these folders is picked up automatically — no code edits:
#   data/ai/     -> AI / synthetic voice   (label 1)
#   data/human/  -> real human voice       (label 0)
# Supported: .mp3 .wav .flac .m4a .ogg .webm .aac
AUDIO_EXTS = (".mp3", ".wav", ".flac", ".m4a", ".ogg", ".webm", ".aac")
for _folder, _label in (("data/ai", 1), ("data/human", 0)):
    os.makedirs(_folder, exist_ok=True)
    for _f in sorted(glob.glob(os.path.join(_folder, "*"))):
        if os.path.splitext(_f)[1].lower() in AUDIO_EXTS:
            LOCAL_FILES.append((_f, _label))

# De-duplicate by path (in case a file is listed twice)
_seen = set()
LOCAL_FILES = [(p, l) for (p, l) in LOCAL_FILES if not (p in _seen or _seen.add(p))]
_n_ai = sum(1 for _, l in LOCAL_FILES if l == 1)
_n_hu = sum(1 for _, l in LOCAL_FILES if l == 0)
print(f"[local data] {len(LOCAL_FILES)} files total  |  AI={_n_ai}  human={_n_hu}  "
      f"(folders data/ai + data/human are auto-loaded)")
...
def main():
    ...
    # Class weights — convention: 0 = real/human, 1 = fake/AI
    n_fake = int(y_train.sum())            # label 1 == fake/AI
    n_real = len(y_train) - n_fake         # label 0 == real/human
    cw = {0: len(y_train)/(2*max(n_real,1)), 1: len(y_train)/(2*max(n_fake,1))}
